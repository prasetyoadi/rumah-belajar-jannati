...
export default function BenefitsManager({ isActive }: BenefitsManagerProps) {
  const { data: benefits, isLoading: isLoadingBenefits } = useBenefits();
  ...
  const getInitialValues = (): Partial<Benefit> => {
    if (editingBenefit) {
      return {
        ...
        isActive:
          editingBenefit.isActive !== undefined
            ? editingBenefit.isActive
            : true,
  ...
  const handleSubmit = async (values: Partial<Benefit>) => {
    try {
      if (editingBenefit) {
        ...
        // Create new benefit
        await createBenefit.mutateAsync(values as Omit<Benefit, 'id' | 'createdAt' | 'updatedAt'>);
      }
      resetForm();
    } catch (error) {
            ...
            {({ isSubmitting: formikSubmitting }) => (
              <Form className="space-y-4">
                ...
                <div className="flex gap-2">
                  <button
                    type="submit"
                    disabled={createBenefit.isPending || updateBenefit.isPending || formikSubmitting}
                  ...
                  >
                    {createBenefit.isPending || updateBenefit.isPending || formikSubmitting
                      ? "Menyimpan..."
                      : editingBenefit
                      ? "Update"
                      : "Simpan"}
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {isLoadingBenefits ? (
                ...
                (benefits || [])
                  .sort((a: Benefit, b: Benefit) => a.order - b.order)
                  .map((benefit: Benefit) => (
                    <tr key={benefit.id}>
